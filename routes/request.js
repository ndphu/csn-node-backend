...
var VungTV = require('../client/VungTv');
...
router.post('', function (req, res, next) {
  ...
  RequestItem.findOne({hash: hash}, function(err, item){
    if (err) {
      console.log(err);
      res.status(500);
      res.send({err: err});
      ...
      VungTV.getItem(url.split('vung.tv')[1]).then(function (result) {
        ...
        var anchor = $('.group-detail a.big-img-film-detail');
        const link = anchor.attr('href');
        const bigPoster = anchor.attr('style').split(/[()]/)[1];
